...
#nullable restore
        ...
        private ArgumentAnalysisResult AnalyzeSpreadArguments(Symbol member, AnalyzedArguments arguments, ArgumentAnalysisResult argumentAnalysis, bool isMethodGroupConversion, ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            ...
            for (var i = 0; i < parameters.Length; ++i)
            {
                ...
                if (param.IsSpread)
                {
                    ...
                    // we need a best guess / attempt at which syntax node to map to the object initializer for the spread param - which should be "parent" of any of the spread args... or actually the "invocation expression"!
                    // anything generated by the spread params will be mapped to the invocation syntax
                    var invocationSyntax = GetInvocationSyntaxFromArgs(arguments);
                    nodeFactory ??= new SyntheticBoundNodeFactory(null, invocationSyntax, null, null);
        ...
        private BoundExpression BuildSpreadArg(Symbol symbol, SpreadArgumentAnalysis spreadArgAnalysis, AnalyzedArguments arguments, SyntheticBoundNodeFactory nodeFactory, ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            ...
            // new instance of the parameter type with an object initializer - mapped to the invocation syntax as the "closes mappable syntax node"
            var initializer = nodeFactory.ObjectInitializer(implicitReceiver, spreadParamType, initExpressions.ToImmutableAndFree());
            return nodeFactory.TryNew((NamedTypeSymbol)spreadParamType)?.UpdateInitializer(initializer) ?? nodeFactory.Null(spreadParamType);
        }

        private static SyntaxNode GetInvocationSyntaxFromArgs(AnalyzedArguments arguments)
        {
            var firstArgParentSyntax = arguments.Argument(0).Syntax.Parent;
            if (firstArgParentSyntax == null)
                firstArgParentSyntax = arguments.Arguments.FirstOrDefault(a => a.Syntax?.Parent != null)?.Syntax.Parent;
            var invocationSyntax = firstArgParentSyntax?.FirstAncestorOrSelf(n => n.IsKind(SyntaxKind.InvocationExpression));
            if (invocationSyntax == null)
                invocationSyntax = firstArgParentSyntax;
